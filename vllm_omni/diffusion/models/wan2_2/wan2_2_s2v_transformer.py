...
class WanS2VRotaryPosEmbed(nn.Module):
    ...
    def forward(self, hidden_states, grid_sizes, trainable_freqs=None):
        ...
        x = hidden_states.detach().view(b, s, self.num_heads, self.head_dim)
        c = self.head_dim // 2
        device = x.device
        ...
        output = torch.view_as_complex(x.reshape(b, s, self.num_heads, -1, 2).to(torch.float64))
        seq_bucket = [0]
...
class WanS2VTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        self_attn_metadata: AttentionMetadata | None = None,
    ):
        seg_boundary = temb[1]
        ...
        # Create attention metadata for self-attention (if not provided)
        if self_attn_metadata is None:
...
class CausalSelfAttention(SimpleSelfAttention):
    def forward(
        ...
        q = self.rotary_embedding(q, grid_sizes_q, freqs, start=start)
        ...
        k = self.rotary_embedding(k, grid_sizes_k, freqs)
...
class WanS2VTransformer3DModel(nn.Module):
    ...
    def after_transformer_block(self, block_idx, hidden_states, merged_audio_emb, audio_emb_global=None):
        if block_idx in self.audio_injector.injected_block_id.keys():
            audio_attn_id = self.audio_injector.injected_block_id[block_idx]
            audio_emb = merged_audio_emb
    ...
    def forward(
        ...
        merged_audio_emb = encoder_hidden_states_audio["audio_emb"]
        audio_emb_global = encoder_hidden_states_audio.get("audio_emb_global") if self.enable_adain else None
        ...
        for idx, block in enumerate(self.blocks):
            ...
            hidden_states = self.after_transformer_block(idx, hidden_states, merged_audio_emb, audio_emb_global)
